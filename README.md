...
**推理**：判据是「单次推理 p99 < 8.33ms（120Hz 单帧预算）」。
BC baseline（0.099M）上 **CPU 完胜**（p99 0.043~0.061ms vs GPU 0.257~0.777ms，
小模型被 kernel launch 开销主导）；但换成真要部署的 **ACT（40.24M）结论直接反转** ——
它每 100 帧才真推理一次，那个尖峰 **CPU p99 39.25ms（超预算 4.7×，会掉帧）
/ GPU p99 2.754ms（占 33%，余量 3×）**。
**部署选型必须按真实策略测，玩具模型会给出相反建议。**

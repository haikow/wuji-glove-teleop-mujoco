...
| 标定用户 + 二代 profile + 二代 MJCF | 0.028~0.040 | **0.0161~0.0178 rad (0.92~1.02°)** | **4 帧 / 33ms** | **0.0** |
...
两边帧集不完全相同。**全部 23 条真机 episode 实测覆盖 96.3%~99.3%（中位 97.5%）**
（其中 3 条 calib 那批是 98.1%~98.4%，之前只报这一批，口径偏窄）。
低于 `min_action_join_ratio`（默认 0.9）判 fail —— 不允许悄悄导出半份数据。
...
过拟合缺口 15.6× → 8.4×（均为 60 epoch 处；40 epoch 处是 5.9×）。
**3 个随机种子复核：40 epoch 的 val_MAE = 0.728° / 0.766° / 0.707°**，
报出的 0.726° 在区间内、可复现。留出 episode 上策略回放与录制动作逐帧吻合（MAE 0.88°）。

⚠️ 之前写的"`val_MAE` 在 24~30 epoch 触底后回升"**不可复现** —— 换种子后有的一路降到
40 epoch。那是单次曲线的假象，这个规模下谈不上明确早停点。
...
| 阶段 | 耗时 | 吞吐 | 备注 |
|---|---|---|---|
| QC | 11.2s | **89 条/s · 26731 帧/s** | 合成 **JSONL** 容器，无真机反馈；真机路径见下 |
...
⚠️ **上面那个 26731 帧/s 不能拿去代表真机数据**。合成 episode 是 JSONL 容器且没有
`hand_state`，而真机路径要解 MCAP、还要跑滞后扫描。同一套 QC 代码，两条路径差 9 倍：

| 路径 | 端到端 QC | 只读解码 | 纯计算 |
|---|---|---|---|
| 合成 JSONL（无真机反馈） | 26 731 帧/s | 45 628 帧/s | 78 989 帧/s |
| **真机 MCAP（23 条 / 29476 帧）** | **2 961 帧/s** | **4 057 帧/s** | **10 460 帧/s** |

两个因子各占一半，机制都查清了：

**解码侧 —— 999Hz 的 `joint_states` 淹没了一切。** 一条 episode 的 MCAP 消息构成：

| topic | 条数 | 占比 | JSON 载荷 |
|---|---|---|---|
| **joint_states** | 10 031 | **80.7%** | **16.3 MB** |
| hand_joint_angles | 1 203 | 9.7% | 0.9 MB |
| hand_skeleton | 1 203 | 9.7% | 4.6 MB |

真机手 999Hz、手套只有 120Hz，**每产出 1 帧要解 10.3 条消息 / 18.2 KB JSON**，
其中 8.3 条 joint_states 最后只被压成 1 个 `hand_state`（取最近时间戳）——**8 条白解**。

**计算侧 —— 滞后扫描是 O(25 档 × 帧数 × 20 关节) 的纯 Python 三重循环。**
去掉 `hand_state` 后纯计算从 10 972 跳到 79 141 帧/s。
...
| 优化前 | 2 988 帧/s |
| 仅向量化滞后扫描 | 3 744（1.29×） |
| 仅按 `log_time` 抽稀 joint_states | 4 233（1.42×） |
| **两项都上** | **6 148（2.06×）** |

- **抽稀**：`joint_states` 按 `log_time` 降到每帧约 2 个再解 JSON（log_time 不用解 JSON 就能拿到）。
  实测 `track_mae_best` 从 0.01783 变 0.01794（+0.6%），最优滞后不变，帧数不变。
- **向量化**：numpy 只作可选加速，没装就回退纯 Python —— 保住"CI 只装标准库也能跑 QC"。
  纯计算 11 849 → 63 708 帧/s（5.4×）。

⚠️ **这里踩了一个很隐蔽的坑**：第一版向量化写完后端到端只快了 4%。profile 发现纯 Python
的 `_mae` 仍占 89% —— numpy 分支**根本没进去**。原因是 join 不上 action 的帧在 `cmd` 里是
`None`（正是 97.5% join 率的体现），`np.asarray(..., dtype=float)` 形状不齐直接抛
`ValueError`，被 `except` 静默吞掉回退了。改成**填 NaN + 掩码**（不能删行，滞后是按帧索引算的）
才真正生效。教训：**带 try/except 回退的优化必须验证快路径真的被走到**，否则只是"看起来优化了"。
...
**中位数 12 140 → 77 807 samples/s，6.41×（5 次重复，区间 5.47~7.00×）。**
上表是单次扫参的示意值，会偏高——最初按单次报的 6.73× 复测后是 6.41×。两个反直觉的点：
...
| 数据 | 优化前 | 优化后 | 倍数 |
|---|---|---|---|
| 真机 19 条 / 22350 帧（3 次中位数） | 2770 帧/s | **12 375 帧/s** | **4.47×** |
| 合成 1000 条 / 30 万帧（单次） | 2391 帧/s | 6846 帧/s | 2.9× |

导出这段是单线程 CPU + 顺序写盘，波动只有约 7%，比训练侧的 IO 配置稳得多。
...
| 模型 | 参数 | 朴素基线¹ | 调优后² | 倍数 | 单次波动 | 最优配置瓶颈 | GPU 利用率 | 峰值显存 |
|---|---|---|---|---|---|---|---|---|
| mlp | 0.10M | 10 078 | 58 700 | **5.82×** | 6~8% | **IO**（89% dataload） | 8.4% → 6.0% | 24 MB |
| big | 12.85M | 10 176 | 49 053 | **4.82×** | 10~11% | **IO**（63% dataload） | 15.6% → 26.3% | 276 MB |
| xl | 118M | 3 765 | **15 578** | **4.14×** | 2~18% | **compute** | 59.8% → 74.5% | 1955 MB |

¹ 朴素基线 = `num_workers=0` + 无 AMP + batch 256，很多人默认就这么写。单位 samples/s。
² **均为 5 次重复的中位数**。

⚠️ **单次测量在这里是不可信的**（这一节最初就栽在这上面）：IO 绑死的配置对页缓存和 CPU
争用极其敏感，同一配置单次之间实测能差 **40%** —— 最初按单次报的 mlp "7.04×" 复测后
中位数只有 5.82×，而 xl 反而被低估（3.67× → 4.14×）。所以 `tools/bench_train.py`
默认 `--repeats 3` 取中位数并同时打印波动区间。compute 绑死的配置稳得多（xl 基线仅 2%）。
...
```
11 008 → 15 578 samples/s (+42%)    optimizer 占比 44.3% → 27.1%    峰值显存 2379 → 1955 MB (-18%)
```

显存那个数在两种计时口径下都精确复现（2379.0 / 1955.4 MB），是确定的；吞吐提升按
中位数是 +42%（最初单次报的是 +14%）。

**复现**（默认 `--repeats 3`，报中位数与波动）：

```bash
./venv312/bin/python tools/bench_train.py --dataset data/datasets/finger_tap \
    --repo-id local/wuji_finger_tap --models mlp big xl --batch-sizes 256 1024 \
    --fused-adam --repeats 5
```
...
| 部署 | p50 | p95 | p99（3 次） | max（3 次） | 占帧预算(p99) |
|---|---|---|---|---|---|
| **CPU** | 0.028~0.037 | 0.033~0.041 | **0.043~0.061 ms** | 0.059~0.351 | **0.5~0.7%** |
| CUDA | 0.060~0.061 | 0.069~0.392 | 0.257~0.777 ms | **1.54~2.51** | 3.1~9.3% |

（最初只跑一次报的是 CPU p99 0.037 / GPU p99 0.094，复跑 3 次发现都偏乐观，
**GPU 尾延迟尤其飘**——这恰恰强化了结论。）

（端到端口径：numpy → tensor → H2D → forward → D2H → numpy，不是只测 forward。）

**结论：CPU 完胜 GPU。** 小模型上一次 H2D + kernel launch 的固定开销盖过计算，
而且 GPU 的**尾延迟又高又不稳**（p99 在 0.26~0.78ms 之间跳，最坏 2.5ms；CPU p99 稳定在
0.04~0.06ms）——实时回路里怕的正是这种抖动。
同理 fp16 在 batch=1 上也是负收益（p50 0.084 vs 0.050，autocast 开销大于收益）。

按 CPU p99 最差的一次（0.061ms）算，余量仍有 **137×**，叠加到 33ms 端到端上只 +0.2%。
**这个策略可以直接塞进回路。**
...
上面那份是 0.099M 的 BC baseline——**它不会上线**。拿真要部署的 ACT 再测一遍
（`tools/bench_policy.py --infer`），结果完全相反。

先看 ACT 的部署机制：`select_action` 维护一个动作队列，**只有队列空时才真跑模型**，
其余帧只是 `popleft()`。所以延迟不是均匀的，而是**每 100 帧一个尖峰**。
决定掉不掉帧的是那个尖峰，不是平均值。

| 部署 | 尖峰 p50 | 尖峰 p99 | 队列帧 p99 | 摊薄/帧 | 尖峰占帧预算 |
|---|---|---|---|---|---|
| CPU | 12.899 | **39.250 ms** | 0.391 | 0.382 | **471%** ← 掉帧 |
| **CUDA** | 2.606 | **2.754 ms** | 0.221 | 0.198 | **33%** ← 塞得进 |

**CPU 的尖峰超出单帧预算 4.7 倍**——每 100 帧就有 1 帧要花 39ms，等于连丢 4 帧。
GPU 尖峰 2.754ms，余量 3 倍。

> **同一个问题，玩具模型和真实策略给出了相反的部署建议。**
> 0.099M 的 MLP 上 GPU 输在 kernel launch 固定开销；40.24M 的 ACT 上这点开销被
> 实际计算量彻底摊薄，CPU 反而扛不动。**部署选型必须按真要上线的模型测。**

另外两点值得记：

- **动作分块把平均成本压得极低**：GPU 摊薄到每帧只 0.198 ms，因为 100 帧只推理 1 次。
  但**不能用摊薄值做准入判据**——掉帧发生在尖峰那一帧。
- 叠加到实测 33ms 端到端遥操延迟上：尖峰帧变成 **35.75 ms（+8.3%）**，其余帧几乎不变。

**可复用的准入判据**（这才是这一节的产出，而不是某个具体数字）：

> 策略的**单次推理 p99 必须 < 8.33 ms**（120Hz 单帧预算）。
> 超了就得异步推理、缩小 chunk、或换更小的模型。

**复现**：

```bash
./venv312/bin/python tools/bench_policy.py --dataset data/datasets/finger_tap \
    --repo-id local/wuji_finger_tap --infer
```
